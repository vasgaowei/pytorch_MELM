...
int roi_ring_pooling_forward(int pooled_height, int pooled_width, float spatial_scale,
                        THFloatTensor * features, THFloatTensor * rois, THFloatTensor * output)
{
    ...
    for (n = 0; n < num_rois; ++n)
    {
        ...
        int roi_start_w_in = round(rois_flat[index_roi + 5] * spatial_scale);
        int roi_start_h_in = round(rois_flat[index_roi + 6] * spatial_scale);
        int roi_end_w_in = round(rois_flat[index_roi + 7] * spatial_scale);
        int roi_end_h_in = round(rois_flat[index_roi + 8] * spatial_scale);
